...
def save_caption_vectors_flowers(data_dir, dt_range=(1, 103)) :
    ...
    target_file_path = os.path.join(data_dir, "Data/Datasets/cars/allclasses.txt")
    caption_dir = join(data_dir, 'Data/Datasets/cars/text_c10')
    ...
    pickle.dump(image_captions,
                open(os.path.join(data_dir, 'cars', 'cars_caps.pkl'), "wb"))

    pickle.dump(tr_image_ids,
                open(os.path.join(data_dir, 'cars', 'cars_ids.pkl'), "wb"))
    pickle.dump(val_image_ids,
                open(os.path.join(data_dir, 'cars', 'val_ids.pkl'), "wb"))

    ec_pkl_path = (join(data_dir, 'cars', 'cars_tv.pkl'))
    pickle.dump(encoded_captions, open(ec_pkl_path, "wb"))

    fc_pkl_path = (join(data_dir, 'cars', 'car_tc.pkl'))
